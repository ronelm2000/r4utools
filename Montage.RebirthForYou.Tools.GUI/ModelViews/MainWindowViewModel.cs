...
namespace Montage.RebirthForYou.Tools.GUI.ModelViews
{
    public class MainWindowViewModel : ReactiveObject
    {
        ...
        private bool IsDeckConstructionValid(CardEntryModel cardEntry)
        {
            ...
            else if (   DeckResults.Where(cr => cr.Card.Type != CardType.Partner).Count() >= 50 || 
                        DeckResults .Where(cr => cr.Card.Type != CardType.Partner)
                                    .Where(cr => cr.Card.Name.AsJPThenEN() == cardEntry.Card.Name.AsJPThenEN())
                                    .Count() >= 4
                                    )
            {
                return false;
